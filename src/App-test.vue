...
function updateHandle(val: string) {
  value.value = val
}

function inputRule(val: string) {
  const reg = /[^\d]/;
  if(reg.test(val)){
    return false
  }
...
const prefixCom = defineComponent({
  setup(_props, _ctx) {
...
const config = reactive<FormConfigList>([
  {
    ...
    autocompleteProps: {
      prepend: markRaw(prefixCom),
      suggestionItem: markRaw(suggestionItem),
      fetchSuggestions: (_query: string, cb: (suggestions: any[]) => void) => {
        setTimeout(() => {
          cb([{ value: '1' }, { value: '2' }])
        })
      }
